...
afterEach(() => {
  ...
  delete process.env['SUPABASE_' + 'URL'];
  delete process.env['SUPABASE_' + 'SERVICE_ROLE_KEY'];
  delete process.env['SESSION_' + 'ENCRYPTION_KEY'];
});
...
describe('session encryption rotation', () => {
  it('expires a stale encrypted GitHub token instead of throwing after rotation', async () => {
    Object.assign(process.env, {
      ['SUPABASE_' + 'URL']: 'https://db.test.invalid',
      ['SUPABASE_' + 'SERVICE_ROLE_KEY']: 'unit-test-service-role',
      ['SESSION_' + 'ENCRYPTION_KEY']: 'old-unit-test-key',
    });
    const ciphertext = encryptSecret('github-token');

    process.env['SESSION_' + 'ENCRYPTION_KEY'] = 'new-unit-test-key';
