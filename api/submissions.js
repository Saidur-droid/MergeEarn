...
import { assertRepoMaintainer, getBounty, getSession, github, handleError, json, method, readJson, requireSession, supabase, transitionBounty } from './_lib/server.js';
...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (req.method === 'GET' && url.searchParams.get('action') === 'checks') {
      ...
      return json(res, 200, {
        state: summarizeChecks(statusResult.data, checkResult.data),
    ...
    if (req.method !== 'POST') return json(res, 404, { error: 'Unknown submissions route.' });
